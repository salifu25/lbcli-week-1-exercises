...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest validateaddress "$P2SH_ADDR" | grep -oP '"isvalid":\s*\K(true|false)')
...
# WRITE YOUR SOLUTION BELOW:
NEW_TAPROOT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress "" bech32m | tr -d '[:space:]')
...
# WRITE YOUR SOLUTION BELOW:
DERIVED_ADDR_RAW=$(bitcoin-cli -regtest deriveaddresses "$TAPROOT_DESCRIPTOR" | grep -oP '"?\K(bcrt1[a-z0-9]+)(?=")')
